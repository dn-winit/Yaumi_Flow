...
def enrich_with_load(
    ...
    *,
    ...
    # the truck from t=0." Demand for the simulation step is the
    # ``predicted_col`` (the model's forecast), so the simulation is
    # internally consistent with the forecast it's reconciling.
    ...
    for d in valid_dates:
        ...
        # Advance leftover with predicted demand; masked rows force
        # demand=0 so prior leftover is preserved (whale absent => no sale).
        for k in range(day_idx.size):
